...
def apply_ordering(queryset, ordering, project, request, view_data=None):
    if ordering:
        ...
        for index, raw_field_name in enumerate(ordering):
            ...
            if field_name.startswith('data__'):
                ...
                annotation_name = f'ordering_field_{index}'  # Unique annotation name

                if numeric_ordering:
                    annotations[annotation_name] = Cast(KeyTextTransform(json_field, 'data'), output_field=FloatField())
                    # For non-numeric values we need fallback to string ordering
                    try:
                        temp_queryset = queryset.annotate(**annotations)
                        temp_queryset.first()
                    except Exception as e:
                        logger.warning(f'Failed to apply numeric ordering for field {json_field}: {e}')
                        annotations[annotation_name] = KeyTextTransform(json_field, 'data')
                else:
                    annotations[annotation_name] = KeyTextTransform(json_field, 'data')

                f = F(annotation_name).asc(nulls_last=True) if ascending else F(annotation_name).desc(nulls_last=True)
            else:
                f = F(field_name).asc(nulls_last=True) if ascending else F(field_name).desc(nulls_last=True)

            order_fields.append(f)

        # Apply annotations and orderings to the queryset
        if annotations:
